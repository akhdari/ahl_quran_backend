<?php 
    ...
    class Teacher {
		...
		/** @var mixed $work_hours */
		public $work_hours;

		/** @var mixed $teacher_contact_id */
		public $teacher_contact_id;

		/** @var mixed $teacher_account_id */
		public $teacher_account_id;
		...
		public static function createAssociatedProfile(DB $db, int $account_id): self {
		    $conn = $db->getConnection();
			$sql = "INSERT INTO " . self::$tableName . " (teacher_account_id) VALUES (?)";
		    $stmt = $conn->prepare($sql);
		    if (!$stmt) throw new \RuntimeException("Prepare failed: " . $conn->error);
		    $stmt->bind_param('i', $account_id);
			$stmt->execute();
			return Teacher::get($db,$db->lastInsert());
		}
		...
		public function update(DB $db): bool {
		    ...
		    $sql = "UPDATE " . self::$tableName .
		           " SET " . implode(', ', $sets) . " WHERE teacher_id = ?";
		    $stmt = $conn->prepare($sql);
		    if (!$stmt) throw new \RuntimeException("Prepare failed: " . $conn->error);
		...
		public function delete(DB $db): bool {
		    $conn = $db->getConnection();
		    $id = $this->teacher_id;
		    $sql = "DELETE FROM " . self::$tableName . " WHERE teacher_id = ?";
		    $stmt = $conn->prepare($sql);
		    if (!$stmt) throw new \RuntimeException("Prepare failed: " . $conn->error);
		    $stmt->bind_param('i', $id);
		    return $stmt->execute();
