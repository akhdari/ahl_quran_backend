<?php 
    ...
    class ContactInfo {
		...
		public static function create(DB $db, array $data): self {
		    ...
		    $stmt->execute();
		    $data['contact_id'] = $conn->insert_id;
		...
		public static function get(DB $db, int ...$id): ?self {
		    $conn = $db->getConnection();
		    $sql = "SELECT * FROM " . self::$tableName . " WHERE contact_id = ?";
		    $stmt = $conn->prepare($sql);
		    if (!$stmt) throw new \RuntimeException("Prepare failed: " . $conn->error);
		...
		public function update(DB $db): bool {
		    ...
		    $sql = "UPDATE " . self::$tableName .
		           " SET " . implode(', ', $sets) . " WHERE contact_id = ?";
		    $stmt = $conn->prepare($sql);
		    if (!$stmt) throw new \RuntimeException("Prepare failed: " . $conn->error);
		...
		public function delete(DB $db): bool {
		    $conn = $db->getConnection();
		    $id = $this->contact_id;
		    $sql = "DELETE FROM " . self::$tableName . " WHERE contact_id = ?";
		    $stmt = $conn->prepare($sql);
		    if (!$stmt) throw new \RuntimeException("Prepare failed: " . $conn->error);
		    $stmt->bind_param('i', $id);
		    return $stmt->execute();
